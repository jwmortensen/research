...
getVarOnDay <- function(ncdf.file, varid, day) {
  ...
  # This transpose is necessary to make sure that values match with hrldas.grid
...
time.temp.data <- vector("list", length=length(unique.days))
names(time.temp.data) <- list.names
time.temp.data <- vector("list", length=1)
for (i in 1:10) { #:length(unique.days)) {
  ...
  time.temperature.data[[list.names[i]]] <- cbind(hrldas.grid, hi_max, hi_max.l1, hi_max.l2, hi_max.l3)[kp.gp,]
  close.ncdf(ncdf.file)
}
